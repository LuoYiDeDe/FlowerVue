...
server {
    ...
    # 静态资源缓存
    location /assets {   位置/资产{
        expires 1y;   过期1 y;
        add_header Cache-Control "public, no-transform";添加标头缓存控制“公共，无转换”；
    }
}
```
...
```bash   ”“bash
# 安装 certbot
sudo apt install certbot python3-certbot-nginx使用 sudo apt 安装 certbot 和 python3-certbot-nginx 。

# 获取 SSL 证书
sudo certbot --nginx -d your-domain.com使用 sudo 命令运行 certbot 并指定 --nginx 参数，同时指定域名 your-domain.com。
```

4. 重启 Nginx
```bash   ”“bash
sudo systemctl restart nginx以超级用户身份运行命令，重启 Nginx 服务。
```
...
# 构建阶段
FROM node:16-alpine as build-stageFROM 指定使用 node:16-alpine 作为构建阶段的基础镜像。
WORKDIR /app
COPY package*.json ./
RUN npm install
COPY . .
...
3. 运行容器
```bash
docker run -d -p 80:80 flower-shop运行容器，后台运行，将容器的 80 端口映射到主机的 80 端口，容器镜像名为 flower-shop ：
